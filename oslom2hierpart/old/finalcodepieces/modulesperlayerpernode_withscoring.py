...
from getnodesperlayermodule import *
from getNneighbors import *
...
#N=5000 #arreglar esto despues! volver a traer el codigo que tenia para calcular N
N,neighbors=getNneighbors(edgesfilename)
layerbasefilename=edgesfilename+"_oslom2modbp_files/xl"
...
for i in range(N):
    ...
    if len(paths)==1:
        ...
        for layer_index in range(number_of_layers): # vamos haciendolo por layer, siempre que len(allowed_paths)>1
            #if len(allowed_paths) <= 1: # if there's only one
            #    break
            ...
            #size_intersection=set(getnodesperlayermodule(edgesfilename,layer_index,path[layer_index])).intersection(neighbors[i])

        finalpath.append(random.choice(allowed_paths))
        #finalpath.append(...) # completar aca!
