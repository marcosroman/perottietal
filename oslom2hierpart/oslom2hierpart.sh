...
oslom2hierpartfolder=~/projects/perottietal/code/oslom2hierpart
...
if [[ $1 != "" ]]; then
	#edgesfile=$1
	#edgesfile=$currfolder/$1
	edgesfile=$(realpath $1)
	nspfile=$(realpath $destfolder)/$(echo $(basename $edgesfile) | sed 's/\.edges$/_oslom.nsp/')
